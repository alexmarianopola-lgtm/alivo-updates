...
POINT_MODULE_VERSION = "0.22.99"
POINT_API_PORT = 17892
POINT_API_HEADER = "X-ALIYVO-Ponto"
POINT_API_SECRET = "aliyvo-ponto-02299"
...
class AliyvoPontoController(QObject):
    ...
    def __init__(self, owner: QWidget, nav_button: QPushButton | None = None):
        ...
        self._log_file = _base_dir() / "logs" / "DIAGNOSTICO_PONTO_02299.jsonl"
    ...
    def punch_ahgora(self) -> None:
        """Abre a página oficial de batida usando a mesma sessão Ahgora já vinculada.

        A empresa deste usuário recusou cadastro de dispositivo por API. Por isso,
        o ALIYVO não tenta criar dispositivo nem enviar a batida por endpoint
        alternativo. O registro é feito pelo próprio portal oficial do Ahgora.
        """
        if self._ahgora_punch_dialog is not None and self._ahgora_punch_dialog.isVisible():
            try:
                self._ahgora_punch_dialog.raise_()
                self._ahgora_punch_dialog.activateWindow()
            except Exception:
                pass
            return
        ...
        dlg = QDialog(self.owner)
        self._ahgora_punch_dialog = dlg
        dlg.setWindowTitle("ALIYVO • Bater ponto no Ahgora")
        dlg.resize(980, 720)
        dlg.setStyleSheet(
            "QDialog{background:#071827;color:#F4FAF8;}"
            "QLabel{color:#DDE9F2;}"
        ...
        info = QLabel(
            "Você está no portal OFICIAL do Ahgora. Faça a batida normalmente nesta página. "
            "O ALIYVO não cria dispositivo e não envia a batida por API paralela."
        )
        info.setWordWrap(True)
        info.setStyleSheet(
            "background:#0B2D27;color:#B9F4D2;border:1px solid #1B8E60;"
            "border-radius:7px;padding:8px;font-size:11px;font-weight:700;"
        )
        root.addWidget(info)
        ...
        details = QLabel(
            ...
            + "   •   Próximo evento: "
            + label
            + "   •   Batidas sincronizadas hoje: "
            + previous
        )
        details.setWordWrap(True)
        details.setStyleSheet("color:#AFC2D0;font-size:10px;padding:2px;")
        ...
        bar = QHBoxLayout()
        reload_btn = QPushButton("↻ Recarregar")
        sync_btn = QPushButton("✅ JÁ BATI NO AHGORA — SINCRONIZAR")
        close_btn = QPushButton("Fechar")
        close_btn.setStyleSheet(
            "QPushButton{background:#142A3B;color:#DCE8F1;border:1px solid #2A465B;"
            "border-radius:7px;padding:9px;font-weight:700;}"
        ...
        def after_sync(ok: bool, msg: str):
            if ok:
                self._log("ahgora_official_punch_synced")
                QMessageBox.information(
                    dlg,
                    "Batidas atualizadas",
                    "O ALIYVO sincronizou novamente o espelho do Ahgora.\n\n"
                    "Confira o novo horário na tela de Controle de Ponto.",
                )
                dlg.close()
                self._rebuild_dialog_if_open()
            else:
                sync_btn.setEnabled(True)
                sync_btn.setText("✅ JÁ BATI NO AHGORA — SINCRONIZAR")
                QMessageBox.warning(
                    dlg,
                    "Ainda não confirmou no espelho",
                    msg
                    + "\n\nSe a batida acabou de ser feita, aguarde alguns segundos e tente sincronizar novamente.",
                )

        def sync_after_punch():
            sync_btn.setEnabled(False)
            sync_btn.setText("Sincronizando espelho...")
            self._sync_using_view(
                view,
                reason="after_official_punch",
                done=after_sync,
            )

        sync_btn.clicked.connect(sync_after_punch)

        def finished(_result=0):
            if self._ahgora_punch_dialog is dlg:
                self._ahgora_punch_dialog = None

        dlg.finished.connect(finished)
        view.setUrl(QUrl(AHGORA_OFFICIAL_PUNCH_PAGE))
        dlg.show()
        try:
            dlg.raise_()
            dlg.activateWindow()
    ...
    def show_dialog(self, manual: bool = True) -> None:
        ...
        if configured and ah.get("last_sync_ok"):
            ...
            always_punch.setToolTip(
                "Abre a página oficial do Ahgora dentro do ALIYVO para você registrar a batida."
            )
            ...
            early_note = QLabel(
                "O botão abre o portal oficial do Ahgora. Depois de bater por lá, clique em "
                "JÁ BATI NO AHGORA — SINCRONIZAR para o ALIYVO conferir o novo horário."
            )
        ...
        if pending:
            ...
            if configured and ah.get("last_sync_ok"):
                info = QLabel(
                    "A batida é feita no portal oficial do Ahgora aberto pelo botão verde acima."
                )
            else:
    ...
    def _start_api(self) -> None:
        ...
        class Handler(BaseHTTPRequestHandler):
            server_version = "ALIYVO-Ponto/0.22.99"
