...
# v0.22.39 - arraste robusto de imagem do WhatsApp/WebView2 para o buscador Somaforce.
# O ALIYVO intercepta o drop no QWebEngineView, transforma imagem/arquivo virtual em arquivo
# temporario e entrega ao input[type=file] da pagina usando chooseFiles do QWebEnginePage.
...
class _AliyvoSomaforceDropFilter(QObject):
        ...
        try:
            ...
            for f in md.formats():
                ...
                if 'image' in s or 'filecontents' in s or 'filegroupdescriptor' in s:
                    return True
            h=str(md.html() or '') if md.hasHtml() else ''
    ...
    def eventFilter(self,obj,event):
        try:
            if not self._inside_browser(obj):
                return False
            et=event.type()
            ...
            if et==QEvent.Type.Drop:
                try:
                    md=event.mimeData()
                    ok=self.panel._aliyvo_handle_photo_drop(md)
                    if ok:
                        event.acceptProposedAction()
                        return True
...
# Instala pagina customizada antes de carregar o site.
...
# Instala filtro global e uma faixa de status curta apos adicionar o browser ao layout.
...
methods=r'''    def _aliyvo_set_drop_status(self,message,error=False):
    ...
    def _aliyvo_save_bytes(self,data):
        try:
            ...
            elif raw[:4] in (b"RIFF",) and b"WEBP" in raw[:16]: ext="webp"
            elif raw[:6] in (b"GIF87a",b"GIF89a"): ext="gif"
            # So aceita conteudo razoavel como imagem/arquivo virtual.
    ...
    def _aliyvo_path_from_mime(self,md):
        # 1) Arquivo normal arrastado do Explorer.
        try:
            if md.hasUrls():
                for u in md.urls():
                    try:
        ...
        # 2) Imagem real entregue pelo Windows/Qt (melhor caso para WhatsApp).
        try:
            if md.hasImage():
                img=md.imageData()
                if img is not None:
        ...
        # 3) Arquivo virtual de drag-and-drop do Chromium/WebView2 no Windows.
        try:
            ...
            for fmt in preferred:
                ...
                try:
                    raw=md.data(fmt)
                    path=self._aliyvo_save_bytes(raw)
                    if path: return path
                except Exception:
                    pass
        except Exception:
        ...
        # 4) data:image embutido em HTML/texto.
        try:
            import base64
            sources=[]
    ...
    def _aliyvo_handle_photo_drop(self,md):
        try:
            ...
            if not path:
                ...
                self._aliyvo_set_drop_status("⚠ O WhatsApp arrastou a foto, mas não entregou os bytes da imagem. Formatos: "+", ".join(fmts[:5]),True)
                return False
    ...
    def _aliyvo_upload_file_to_site(self,path):
        try:
            ...
            if page is None or not hasattr(page,"set_pending_upload"):
                self._aliyvo_set_drop_status("⚠ O navegador interno não está preparado para receber o arquivo.",True)
                return
            ...
            def after(res):
                try:
                    ...
                    ok=False
                if not ok:
                    self._aliyvo_set_drop_status("⚠ Não encontrei o campo de arquivo do site. Recarregue o buscador e tente novamente.",True)
                    return
                QTimer.singleShot(1100,self._aliyvo_submit_photo_request)
    ...
    def _aliyvo_submit_photo_request(self):
        try:
            js=r'''(() => {
              ...
              if(!b) return {ok:false,reason:'send_button_not_found'};
              b.click();
              return {ok:true};
            })()'''
            def done(res):
