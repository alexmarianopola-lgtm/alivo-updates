...
POINT_MODULE_VERSION = "0.22.96"
POINT_API_PORT = 17892
POINT_API_HEADER = "X-ALIYVO-Ponto"
POINT_API_SECRET = "aliyvo-ponto-02296"
...
class AliyvoPontoController(QObject):
    ...
    def __init__(self, owner: QWidget, nav_button: QPushButton | None = None):
        ...
        self._log_file = _base_dir() / "logs" / "DIAGNOSTICO_PONTO_02296.jsonl"
    ...
    def _sync_using_view(
        ...
        *,
        ...
        """Sincroniza usando a sessão real do portal, sem reproduzir o login."""
        if self._sync_busy:
            if done:
                done(False, "Já existe uma sincronização em andamento.")
            return
        self._sync_busy = True
        self._log("ahgora_browser_sync_started", reason=reason)
        ...
        def fetch_index():
            ...
            if host != "app.ahgora.com.br":
                fail(
                    "O portal ainda não está autenticado no app.ahgora.com.br. "
                    "Conclua o login do Ahgora nesta janela."
                )
                return
            ...
            (async () => {
              try {
                ...
                return {ok:r.ok, status:r.status, text:text, href:location.href};
              } catch (e) {
                return {ok:false, status:0, error:String(e), href:location.href};
              }
            })();
            """
            ...
            def on_index(result):
                if not isinstance(result, dict) or not result.get("ok"):
                    ...
                    fail(
                        f"A sessão do Ahgora não liberou o espelho (HTTP {status}). "
                        + (str(err) if err else "")
                    )
                    return
                try:
                    index = json.loads(str(result.get("text") or ""))
                except Exception:
                    fail("O Ahgora respondeu ao espelho, mas não retornou JSON válido.")
                    return
                months = index.get("meses") if isinstance(index, dict) else None
                if not isinstance(months, dict):
                    fail(
                        "O Ahgora abriu, mas a sessão atual não tem acesso ao espelho de ponto."
                    )
                    return
                ...
                def on_detail(detail_result):
                    if not isinstance(detail_result, dict) or not detail_result.get("ok"):
                        status = detail_result.get("status") if isinstance(detail_result, dict) else "?"
                        fail(f"O Ahgora não liberou o detalhe do espelho (HTTP {status}).")
                        return
                    try:
                        detail = json.loads(str(detail_result.get("text") or ""))
                ...
                view.page().runJavaScript(js_detail, on_detail)

            view.page().runJavaScript(js, on_index)
        ...
        def on_loaded(ok: bool):
            try:
                ...
                pass
            if not ok:
                fail("Não foi possível abrir o portal do Ahgora.")
                return
            # Se a sessão é válida, o mirror permanece em app.ahgora.com.br.
            fetch_index()

        try:
            view.loadFinished.connect(on_loaded)
            view.setUrl(QUrl(AHGORA_MIRROR_HOME))
        except Exception as exc:
            fail("Falha ao abrir sessão do Ahgora: " + str(exc))
    ...
    def _start_api(self) -> None:
        ...
        class Handler(BaseHTTPRequestHandler):
            server_version = "ALIYVO-Ponto/0.22.96"
