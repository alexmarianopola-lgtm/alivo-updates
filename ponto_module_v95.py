...
POINT_MODULE_VERSION = "0.22.95"
POINT_API_PORT = 17892
POINT_API_HEADER = "X-ALIYVO-Ponto"
POINT_API_SECRET = "aliyvo-ponto-02295"
...
class AliyvoPontoController(QObject):
    ...
    def __init__(self, owner: QWidget, nav_button: QPushButton | None = None):
        ...
        self._log_file = _base_dir() / "logs" / "DIAGNOSTICO_PONTO_02295.jsonl"
    ...
    def show_ahgora_settings(self) -> None:
        ...
        hint = QLabel(
            "O ALIYVO usa o login web atual do Ahgora e o novo espelho /api-espelho. "
            "Salvar e sincronizar NÃO bate ponto. A Batida Online só é enviada quando você clicar "
            "no botão BATER PONTO e confirmar."
        )
    ...
    def sync_ahgora_async(self, reason: str = "manual") -> None:
        ...
        self._log("ahgora_sync_started", reason=reason, api="modern_mirror")

        def worker():
            try:
                cfg = self._load_credentials()
                now = datetime.now()
                result = _login_and_fetch_modern_mirror(cfg, now)
    ...
    def show_dialog(self, manual: bool = True) -> None:
        ...
        def show_sync_diag():
            ...
            lines = [
                ...
                "Sessão Ahgora válida: " + ("SIM" if validation.get("session_ok") else "-"),
    ...
    def _start_api(self) -> None:
        ...
        class Handler(BaseHTTPRequestHandler):
            server_version = "ALIYVO-Ponto/0.22.95"
