...
POINT_MODULE_VERSION = "0.22.98"
POINT_API_PORT = 17892
POINT_API_HEADER = "X-ALIYVO-Ponto"
POINT_API_SECRET = "aliyvo-ponto-02298"
...
class AliyvoPontoController(QObject):
    ...
    def __init__(self, owner: QWidget, nav_button: QPushButton | None = None):
        ...
        self._log_file = _base_dir() / "logs" / "DIAGNOSTICO_PONTO_02298.jsonl"
    ...
    def punch_ahgora(self) -> None:
        ...
        punches_today = [str(x) for x in (self._state.get("ahgora") or {}).get("punches_today") or []]
        previous = " • ".join(punches_today) if punches_today else "nenhuma batida sincronizada"
        ans = QMessageBox.question(
            self.owner,
            "Registrar ponto agora?",
            f"O ALIYVO vai enviar AGORA uma batida REAL para o Ahgora.\n\n"
            f"Horário deste computador: {now.strftime('%H:%M:%S')}\n"
            f"Próximo evento: {label}\n"
            f"Batidas já encontradas hoje: {previous}\n\n"
            "Isso pode criar uma nova batida mesmo antes do horário previsto.\n"
            "Confirme somente se você realmente quer bater o ponto agora.",
    ...
    def show_dialog(self, manual: bool = True) -> None:
        ...
        if configured and ah.get("last_sync_ok"):
            ...
            always_punch.setToolTip(
                "Registra uma batida REAL no Ahgora após sua confirmação, mesmo antes do horário previsto."
            )
            ...
            early_note = QLabel(
                "Você pode bater antes/depois do horário previsto. O ALIYVO sempre pedirá confirmação "
                "e depois sincronizará o Ahgora para conferir a nova batida."
            )
        ...
        if pending:
            ...
            if configured and ah.get("last_sync_ok"):
                info = QLabel(
                    "A batida pode ser feita pelo botão verde acima. Ela só é enviada após sua confirmação."
                )
            else:
    ...
    def _start_api(self) -> None:
        ...
        class Handler(BaseHTTPRequestHandler):
            server_version = "ALIYVO-Ponto/0.22.98"
