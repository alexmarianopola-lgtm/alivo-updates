...
POINT_MODULE_VERSION = "0.23.01"
POINT_API_PORT = 17892
POINT_API_HEADER = "X-ALIYVO-Ponto"
POINT_API_SECRET = "aliyvo-ponto-02301"
...
AHGORA_OFFICIAL_PUNCH_PAGE = "https://www.ahgora.com.br/externo/batidas"
...
def _legacy_modern_punch_unused(cfg: dict[str, str]) -> tuple[dict[str, Any], dict[str, str]]:
...
class AliyvoPontoController(QObject):
    ...
    O ALIYVO nunca registra ponto sozinho. O botão de batida apenas traz o Chrome
    para frente e aciona o atalho configurado para a extensão oficial
    Ahgora Ponto Online (Ctrl+Shift+9). A confirmação continua sendo feita pelo usuário.
    """
    ...
    def __init__(self, owner: QWidget, nav_button: QPushButton | None = None):
        ...
        self._log_file = _base_dir() / "logs" / "DIAGNOSTICO_PONTO_02301.jsonl"
        ...
        self._ahgora_punch_dialog: QDialog | None = None
        self._awaiting_chrome_punch = False
        self._punch_sync_attempts = 0
    ...
    def _on_application_state(self, state) -> None:
        try:
            if state != Qt.ApplicationState.ApplicationActive:
                return
            if self._awaiting_chrome_punch:
                QTimer.singleShot(350, lambda: self._verify_after_chrome_punch("focus_return"))
                return
    ...
    def _find_chrome_window(self) -> int:
        ...
        def enum_proc(hwnd, _lparam):
            try:
                ...
                txt = title.value
                if cls.startswith("Chrome_WidgetWin_") and txt:
                    found["hwnd"] = int(hwnd)
                    return False
            except Exception:
                pass
        ...
        return int(found["hwnd"] or 0)

    def _send_ahgora_chrome_hotkey(self) -> bool:
        """Ativa Chrome e envia Ctrl+Shift+9 usando SendInput após confirmar o foco."""
        if os.name != "nt":
            return False
        ...
        try:
            ...
            # O Windows pode recusar a troca de foco no primeiro instante.
            # Tentamos por até ~1,8 s antes de enviar o atalho.
            focused = False
            for _ in range(9):
            ...
            if not focused:
                self._log(
                    "ahgora_chrome_focus_not_confirmed",
            ...
            sent = int(user32.SendInput(6, ctypes.byref(sequence), ctypes.sizeof(INPUT)))
            self._log(
                "ahgora_chrome_hotkey_sent",
                sent_inputs=sent,
                target_hwnd=int(hwnd),
            )
            return sent == 6
        except Exception as exc:
            self._log("ahgora_chrome_hotkey_error", error=repr(exc))
            return False

    def punch_ahgora(self) -> None:
        """Abre a extensão oficial licenciada Ahgora Ponto Online no Chrome."""
        now = datetime.now()
        pending = pending_slot_for(now, self._state)
        next_slot = next_future_slot_for(now, self._state)
        if pending:
            label = pending["label"] + " • previsto " + pending["time"]
        elif next_slot:
            label = next_slot["label"] + " • previsto " + next_slot["time"] + " (adiantado)"
        else:
            label = "batida extra / fora da previsão"

        punches_today = [
            str(x) for x in (self._state.get("ahgora") or {}).get("punches_today") or []
        ]
        previous = " • ".join(punches_today) if punches_today else "nenhuma"

        ans = QMessageBox.question(
            self.owner,
            "Abrir Ahgora Ponto Online?",
            f"O ALIYVO vai abrir a extensão OFICIAL Ahgora Ponto Online no Chrome.\n\n"
            f"Horário agora: {now.strftime('%H:%M:%S')}\n"
            f"Próximo evento: {label}\n"
            f"Batidas sincronizadas hoje: {previous}\n\n"
            "Na extensão, informe matrícula/senha e clique em Bater Ponto.\n"
            "O ALIYVO não preencherá sua senha e não clicará em Bater Ponto por você.\n\n"
            "Abrir a extensão agora?",
        ...
        self._awaiting_chrome_punch = True
        self._punch_sync_attempts = 0
        self._log(
            "ahgora_chrome_extension_requested",
            shortcut="Ctrl+Shift+9",
            expected_slot=pending["id"] if pending else (next_slot["id"] if next_slot else ""),
        )

        # A troca de foco precisa ocorrer depois que a caixa de confirmação fechar.
        def trigger():
            if not self._send_ahgora_chrome_hotkey():
                self._awaiting_chrome_punch = False
                QMessageBox.warning(
                    self.owner,
                    "Chrome não encontrado",
                    "Não consegui ativar o Chrome e enviar o atalho.\n\n"
                    "Deixe o Chrome aberto e confirme que Ctrl+Shift+9 abre manualmente a extensão Ahgora Ponto Online.",
                )
                return
            # Verificação somente leitura; não cria batida.
            QTimer.singleShot(12_000, lambda: self._verify_after_chrome_punch("timer_12s"))
            QTimer.singleShot(28_000, lambda: self._verify_after_chrome_punch("timer_28s"))

        QTimer.singleShot(250, trigger)

    def _verify_after_chrome_punch(self, reason: str) -> None:
        if not self._awaiting_chrome_punch:
            return
        if self._sync_busy:
            QTimer.singleShot(2_000, lambda: self._verify_after_chrome_punch(reason + "_retry"))
            return

        before = len((self._state.get("ahgora") or {}).get("punches_today") or [])
        self._punch_sync_attempts += 1

        # A rotina existente de sincronização é somente leitura.
        self.sync_ahgora_async(reason="chrome_punch_" + reason)

        def evaluate():
            after = len((self._state.get("ahgora") or {}).get("punches_today") or [])
            if after > before:
                self._awaiting_chrome_punch = False
                self._log(
                    "ahgora_chrome_punch_detected",
                    before_count=before,
                    after_count=after,
                    attempt=self._punch_sync_attempts,
                )
                QMessageBox.information(
                    self.owner,
                    "Ponto confirmado",
                    "O ALIYVO encontrou uma nova batida no espelho do Ahgora.\n\n"
                    "O Controle de Ponto já foi atualizado.",
                )
                self._rebuild_dialog_if_open()
            elif self._punch_sync_attempts >= 3:
                self._awaiting_chrome_punch = False
                self._log("ahgora_chrome_punch_not_detected", attempts=self._punch_sync_attempts)
                QMessageBox.information(
                    self.owner,
                    "Ainda não apareceu no espelho",
                    "Ainda não encontrei uma nova batida no Ahgora.\n\n"
                    "Se você acabou de bater o ponto, pode haver alguns segundos de atraso. "
                    "Use SINCRONIZAR AHGORA para conferir novamente.",
                )

        QTimer.singleShot(3_500, evaluate)
    ...
    def show_dialog(self, manual: bool = True) -> None:
        ...
        if configured and ah.get("last_sync_ok"):
            next_expected = pending or next_future_slot_for(now, self._state)
            if next_expected:
                punch_caption = (
                    "🟢  BATER PONTO AGORA  •  "
                    + now.strftime("%H:%M")
                ...
                punch_caption = "🟢  BATER PONTO AGORA  •  " + now.strftime("%H:%M")
            always_punch = QPushButton(punch_caption)
            always_punch.setToolTip(
                "Abre a extensão oficial Ahgora Ponto Online no Chrome usando Ctrl+Shift+9."
            )
            always_punch.setStyleSheet(
                "QPushButton{background:#087047;color:white;border:1px solid #20E983;"
                "border-radius:8px;padding:12px;font-size:12px;font-weight:900;}"
                "QPushButton:hover{background:#0A8354;}"
            )
            always_punch.clicked.connect(self.punch_ahgora)
            lay.addWidget(always_punch)

            early_note = QLabel(
                "O botão abre a extensão oficial Ahgora Ponto Online no Chrome (Ctrl+Shift+9). "
                "Depois da sua batida, o ALIYVO tenta sincronizar automaticamente o espelho."
            )
            early_note.setWordWrap(True)
            early_note.setStyleSheet("color:#9FCBB7;font-size:10px;padding:1px 2px 3px 2px;")
            lay.addWidget(early_note)
        ...
        if pending:
            ...
            if configured and ah.get("last_sync_ok"):
                info = QLabel(
                    "A batida é feita pela extensão oficial Ahgora Ponto Online no Chrome."
                )
            else:
    ...
    def _start_api(self) -> None:
        ...
        class Handler(BaseHTTPRequestHandler):
            server_version = "ALIYVO-Ponto/0.23.01"
