...
def reconcile_real_punches(state: dict[str, Any], punches: list[str], now: datetime) -> dict[str, Any]:
    ...
    for index, slot in enumerate(POINT_SLOTS):
        ...
        if index < len(punches):
            ...
            # Mantemos "missed" explícito, mas removemos confirmações manuais/antigas.
            if existing.get("status") == "missed":
                continue
...
class AliyvoPontoController(QObject):
    ...
    def __init__(self, owner: QWidget, nav_button: QPushButton | None = None):
        ...
        self._log_file = _base_dir() / "logs" / "DIAGNOSTICO_PONTO_02293.jsonl"
    ...
    def sync_ahgora_async(self, reason: str = "manual") -> None:
        ...
        def worker():
            try:
                ...
                self.syncFinished.emit(True, {"payload": payload, "punches": punches, "reason": reason}, "")
            except Exception as exc:
    ...
    def _on_sync_finished(self, ok: bool, data: object, error: str) -> None:
        ...
        if ok and isinstance(data, dict):
            ...
            self._log("ahgora_sync_ok", punches_count=len(punches), reason=str(data.get("reason") or ""))
        else:
    ...
    def show_dialog(self, manual: bool = True) -> None:
        ...
        if configured and ah.get("last_sync_ok"):
            sync_status = QLabel("● Ahgora conectado • batidas reais sincronizadas")
        ...
        if pending:
            ...
            if configured:
                ...
                setup = QPushButton("⚙  CONECTAR AO AHGORA")
    ...
    def _start_api(self) -> None:
        ...
        class Handler(BaseHTTPRequestHandler):
            server_version = "ALIYVO-Ponto/0.22.93"
